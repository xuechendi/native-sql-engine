...
import org.apache.spark.sql.catalyst.ColumnarBatch
...
object ColumnarGroupedIterator {
  def apply(
      input: Iterator[ColumnarBatch],
      keyExpressions: Seq[Expression],
      inputSchema: Seq[Attribute]): Iterator[(ColumnarBatch, Iterator[ColumnarBatch])] = {
    if (input.hasNext) {
      new ColumnarGroupedIterator(input, keyExpressions, inputSchema)
    } else {
      Iterator.empty
    }
  }
...
/**
 * Iterates over a presorted set of columnar_batches, chunking it up by the grouping expression.  Each call to
 * next will return a pair containing the current group and an iterator that will return all the
 * elements of that group.  Iterators for each group are lazily constructed by extracting columnar_batches
 * from the input iterator.  As such, full groups are never materialized by this class.
 *
 * Example input:
 * {{{
 *   Input: Iterator([[a, b, b], [1, 2, 3]])
 *   Grouping: x#1
 *   InputSchema: x#1, y#2
 * }}}
 *
 ...
 *   Second call to next(): ([b], Iterator([[b, b], [2, 3]])
 * }}}
 *
 ...
 *
 * @param input An iterator of columnar_batches.  This iterator must be ordered by the groupingExpressions or
 ...
 *                            to `next()`.
 * @param inputSchema The schema of the columnar_batches in the `input` iterator.
 */
class ColumnarGroupedIterator (
    input: Iterator[ColumnarBatch],
    groupingExpressions: Seq[Expression],
    inputSchema: Seq[Attribute])
  extends Iterator[(ColumnarBatch, Iterator[ColumnarBatch])] {
    val codeGenNode = ColumnarHashAggregation.prepareKernelFunction(
      groupingExpressions,
      child.output,
...
//  /** Compares two input columnar_batches and returns 0 if they are in the same group. */
...
//
//  def next(): (ColumnarBatch, Iterator[ColumnarBatch]) = {
...
//  }
//
//  private def createGroupValuesIterator(): Iterator[ColumnarBatch] = {
